...
FFTLENGTH = [(0,4,2), (1,4,4), (2,8,2), (3,8,4)]
...
for i,fftlength,fduration in FFTLENGTH:
    print(f"fftlengh={fftlength},fduration={fduration}")
    fftsetting = f"_{fftlength}_{fduration}_"
    output1 = event + fftsetting + "kstest_L1.png" #ksテスト
    output2 = event + fftsetting + "chi2test_L1.png" #アウトプットのファイル
    output3 = event + fftsetting + "adtest_L1.png" #adテスト
    output4 = event + fftsetting + "pvalue_hist_L1.png" #histgram
    output5 = event + fftsetting + "dist_check_L1.png" #分布の比較
    output6 = event + fftsetting + "ratio_L1.txt" #裾の部分の定量化
    output7 = event + fftsetting + "tile_energy_L1.png" #タイルのエネルギー
    output8 = event + fftsetting + "a2_L1.png" #A2自体のプロット
    output9 = event + fftsetting + "ratio_L1_sim.txt"
    output10 = f"_{fftlength}_{fduration}_"
    white = data_L1.whiten(fftlength=fftlength, overlap=fftlength/2,fduration=fduration) #ホワイトニング
    #シミュレーションデータ
    # --- 合成ガウスノイズ（1つ目のセグメント）---
    sim = generate_PSD_gauss(data_L1)
    white_sim = sim.whiten(fftlength=fftlength, overlap=fftlength/2,fduration=fduration)
    ...
    kslist,chi2list,adlist,random_time_list,a2list,all_y,qgram_L1,y_norm = run_test(Q,random_time,white,skipped,geocent_time,rate_of_mabiki,A2_null,seg_duration)
    kslist_sim,chi2list_sim,adlist_sim,random_time_list_sim,a2list_sim,all_y_sim,qgram_L1_sim,y_norm_sim = run_test(Q,random_time,white_sim,skipped,geocent_time,rate_of_mabiki,A2_null,seg_duration)
    plot(kslist,chi2list,adlist,random_time_list,a2list,all_y,qgram_L1,y_norm,kslist_sim,chi2list_sim,adlist_sim,random_time_list_sim,a2list_sim,all_y_sim,qgram_L1_sim,y_norm_sim,geocent_time,event,det,A2_null,output1,output2,output3,output4,output5,output6,output7,output8,output9,output10)