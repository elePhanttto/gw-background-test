...
output1 = event + "heat_map_H1.png" #タイルのエネルギー
...
axes[0].set_xlabel("times[s]"); axes[0].set_ylabel("frequency[Hz]")
axes[0].set_title("real background")

# --- ACF: 全周波数行を重ね書き ---
for k, (freq, acf) in enumerate(acf_dict_real.items()):
    axes[1].plot(acf, color="blue", alpha=0.25, lw=0.8,
                 label="real" if k == 0 else None)

axes[1].axhline(0, c="gray", lw=0.5)
axes[1].set_xlabel("tile lag"); axes[1].set_ylabel("autocorrelation")
axes[1].set_xlim(0, 20)
axes[1].legend()
axes[1].set_title(f"ACF of all frequency rows (n_real={len(acf_dict_real)})")
...
if mean_real is not None:
    axes[1].plot(mean_real, color="blue", lw=2.5, label="real (mean)")
